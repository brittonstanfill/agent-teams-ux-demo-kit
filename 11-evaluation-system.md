...
| Responsive render floor | If the artifact is visual UI, check at least one narrow mobile viewport around 390px wide and one desktop viewport. There should be no horizontal clipping/scroll, incoherent overlap, or text escaping buttons, cards, navigation, or fixed-format mockups. The check must exercise the top-level artifact page; iframe-only probes are useful backup evidence but do not by themselves prove the shipped page fits. If global CSS such as `overflow-x: hidden` could mask layout overflow, rerun the DOM overflow check with that masking disabled or explicitly record why it is not applicable. A top-level `scrollWidth` pass is necessary but not sufficient: if content is rendered inside fixed-format containers such as phone frames, cards, tables, or mockup screens, inspect those containers too. Visible text, controls, or placeholder tokens escaping or clipping inside the container is a gate failure even when the page itself has no horizontal scroll. |
