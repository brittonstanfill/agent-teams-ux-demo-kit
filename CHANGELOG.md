...
- Deck (`deck/agent-teams-meetup-deck.html`) updated to v3+: baseline-prompt slide and team-prompt slide now show the current single-baseline / team-relay content (clean-room rule, HTML artifact requirement, Creative Director role, required debates); prompt-anatomy slide reflects the 9-agent team; resources slide replaced the dead `agent-teams-meetup-plan.md` link with `START_HERE.md` and `bin/install-agents.sh`. The healthcare-scenario slides (Setup, Patient Moment, How we judged it, What just happened) are left as-is — they describe a separate stage demo and are not part of the UX kit.
